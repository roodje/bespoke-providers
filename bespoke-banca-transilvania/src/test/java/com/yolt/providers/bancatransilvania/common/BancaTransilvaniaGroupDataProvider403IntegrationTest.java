 ...
 */
@AutoConfigureWireMock(stubs = "classpath:/stubs/unhappy-flow/accounts-403/", httpsPort = 0, port = 0)
class BancaTransilvaniaGroupDataProvider403IntegrationTest extends BancaTransilvaniaGroupDataProviderTestBaseSetup {

    @Test
    void shouldHandleForbiddenCorrectly() {
        ...
        assertThatThrownBy(fetchDataCallable)
                .isInstanceOfSatisfying(TokenInvalidException.class, exception -> assertThat(exception.getMessage()).isEqualTo("Access to call is forbidden: HTTP 403"));
    }
}