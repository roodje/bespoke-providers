...
@Configuration
public class BnlBeanConfigV2 {

    @Bean("BnlAuthenticationServiceV2")
    public CbiGlobeAuthorizationService getAuthorizationService(BnlProperties properties, Clock clock) {
        return new CbiGlobeAuthorizationService(properties, clock);
    }

    @Bean("BnlConsentRequestServiceV4")
    public CbiGlobeConsentRequestServiceV4 getConsentRequestServiceV4(BnlProperties properties, Clock clock) {
        return new CbiGlobeConsentRequestServiceV4(properties, clock, new CurrentAccountConsentAccessCreator());
    }

    @Bean("BnlHttpClientFactoryV2")
    public CbiGlobeHttpClientFactory getHttpClientFactory(BnlProperties properties,
                                                          @Qualifier("CbiGlobe") final ObjectMapper objectMapper) {
        return new CbiGlobeHttpClientFactory(properties, objectMapper);
    }

    @Bean("BnlFetchServiceV4")
    public CbiGlobeFetchDataServiceV3 getFetchServiceV3(BnlProperties properties, Clock clock) {
