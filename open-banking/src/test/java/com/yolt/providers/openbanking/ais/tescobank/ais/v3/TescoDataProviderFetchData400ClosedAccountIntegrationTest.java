...
@AutoConfigureWireMock(stubs = "classpath:/stubs/tescobank/ais-3.1/accounts-400-blocked", httpsPort = 0, port = 0)
@ActiveProfiles("tescobank")
public class TescoDataProviderFetchData400ClosedAccountIntegrationTest {
    ...
    public void shouldThrowTokenInvalidExceptionWhenRecivedHttp400FromAccountsEndpointWithInvalidAccountStatusMessage(GenericBaseDataProviderV2 provider) throws IOException, URISyntaxException, TokenInvalidException, ProviderFetchDataException {
        // given
        ...
        // when
        ThrowableAssert.ThrowingCallable callable = () -> provider.fetchData(urlFetchData);

        // then
        assertThatExceptionOfType(TokenInvalidException.class)
                .isThrownBy(callable)
                .withMessage("User account has invalid status");
    }
