...
/**
 * This test contains missing data flow (no data in bank's response) occurring in Monzo bank provider.
 */
@SpringBootTest(classes = {MonzoApp.class, OpenbankingConfiguration.class}, webEnvironment = SpringBootTest.WebEnvironment.NONE)
@AutoConfigureWireMock(stubs = "classpath:/stubs/monzogroup/ob_3.1/ais/missing-data/", httpsPort = 0, port = 0)
@ActiveProfiles("monzogroup")
public class MonzoDataProviderV5MissingDataIntegrationTest {
    ...
    @Test
    public void shouldReturnCorrectlyFetchDataWhenSomeDataInBankResponseIsMissing() throws Exception {
        // given
        ...
        assertThat(account.getName()).isEqualTo("Monzo Account");
        ...
        assertThat(account.getExtendedAccount().getBalances().get(0).getBalanceAmount().getAmount()).isNull();
