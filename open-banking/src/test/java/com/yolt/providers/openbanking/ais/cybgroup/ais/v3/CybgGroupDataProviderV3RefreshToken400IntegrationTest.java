...
@AutoConfigureWireMock(stubs = "classpath:/stubs/cybgroup/ais-3.1/grant-type-refresh-token-400", httpsPort = 0, port = 0)
@ActiveProfiles("cybgroup")
public class CybgGroupDataProviderV3RefreshToken400IntegrationTest {
    ...
    void shouldThrowTokenInvalidExceptionWhenRefreshTokenExpired(UrlDataProvider provider) throws JsonProcessingException {
        // given
    ...
    private AccessMeansDTO createAccessMeansDTOWithMissingRefreshToken(List<OBAccount6> accounts) throws JsonProcessingException {
        AccessMeans accessMeans = new AccessMeans(Instant.now(), USER_ID, ACCESS_TOKEN, REFRESH_TOKEN, getExpirationDate(), new Date(), REDIRECT_URL);
