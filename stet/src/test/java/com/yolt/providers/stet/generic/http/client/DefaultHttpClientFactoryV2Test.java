...
class DefaultHttpClientFactoryV2Test {
    ...
    @InjectMocks
    private DefaultHttpClientFactoryV2 httpClientFactory;
    ...
    void shouldReturnConfiguredRestTemplate() {
        ...
        when(restTemplateManager.manage(any(UUID.class), any(X509Certificate[].class), ArgumentMatchers.<Function<ExternalRestTemplateBuilderFactory, RestTemplate>>any()))
        ...
        verify(restTemplateManager).manage(
                eq(CLIENT_TRANSPORT_KEY_ID),
                eq(new X509Certificate[]{CLIENT_TRANSPORT_CERTIFICATE}),
                ArgumentMatchers.<Function<ExternalRestTemplateBuilderFactory, RestTemplate>>any());
    }
    ...
    void shouldReturnConfiguredHttpClient() {
        ...
        // then
        assertThat(httpClient).isInstanceOf(DefaultHttpClientV2.class);
        verify(restTemplateManager).manage(
                eq(CLIENT_TRANSPORT_KEY_ID),
                eq(new X509Certificate[]{CLIENT_TRANSPORT_CERTIFICATE}),
                ArgumentMatchers.<Function<ExternalRestTemplateBuilderFactory, RestTemplate>>any());
