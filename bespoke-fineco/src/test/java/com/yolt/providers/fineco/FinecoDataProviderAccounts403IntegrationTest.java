...
/**
 * This test contains case when request to get accounts returned 403 - Forbidden.
 * This means that request is forbidden, thus we want to force user to fill a consent (so throw {@link TokenInvalidException})
 */

@TestInstance(TestInstance.Lifecycle.PER_CLASS)
@SpringBootTest(classes = FinecoTestApp.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
@AutoConfigureWireMock(stubs = "classpath:/mappings/ais/accounts-403", httpsPort = 0, port = 0)
@ActiveProfiles("fineco")
public class FinecoDataProviderAccounts403IntegrationTest {
    ...
    void shouldThrowTokenInvalidExceptionWhenAccountsRequestFail(UrlDataProvider providerUnderTest) {
        // given
