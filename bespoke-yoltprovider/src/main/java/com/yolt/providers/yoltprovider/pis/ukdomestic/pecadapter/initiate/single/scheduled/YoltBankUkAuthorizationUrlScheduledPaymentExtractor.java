...
public class YoltBankUkAuthorizationUrlScheduledPaymentExtractor implements PaymentAuthorizationUrlExtractor<InitiatePaymentConsentResponse, YoltBankUkInitiateScheduledPaymentPreExecutionResult> {
    ...
    public YoltBankUkAuthorizationUrlScheduledPaymentExtractor(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    @Override
    public String extractAuthorizationUrl(InitiatePaymentConsentResponse initiatePaymentConsentResponse, YoltBankUkInitiateScheduledPaymentPreExecutionResult result) {
        if (initiatePaymentConsentResponse == null) {
            return createUrl(result);
        }
        OBWriteDomesticScheduledConsentResponse1 consentResponse = readConsentResponse(initiatePaymentConsentResponse);
        return createUrl(result, consentResponse.getData().getConsentId(), initiatePaymentConsentResponse.getConsentUri());
    }

    private OBWriteDomesticScheduledConsentResponse1 readConsentResponse(final InitiatePaymentConsentResponse initiatePaymentConsentResponse) {
        try {
            return objectMapper.readValue(initiatePaymentConsentResponse.getPaymentConsent(), OBWriteDomesticScheduledConsentResponse1.class);
        } catch (JsonProcessingException e) {
            throw new IllegalStateException(e);
        }
    }

    private String createUrl(YoltBankUkInitiateScheduledPaymentPreExecutionResult result, String consentId, String consentUri) {
    ...
    private String createUrl(YoltBankUkInitiateScheduledPaymentPreExecutionResult result) {
