...
public class YoltBankUkAuthorizationUrlSinglePaymentExtractor implements PaymentAuthorizationUrlExtractor<InitiatePaymentConsentResponse, YoltBankUkInitiateSinglePaymentPreExecutionResult> {
    ...
    public YoltBankUkAuthorizationUrlSinglePaymentExtractor(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    @Override
    public String extractAuthorizationUrl(InitiatePaymentConsentResponse initiatePaymentConsentResponse, YoltBankUkInitiateSinglePaymentPreExecutionResult result) {
        if (initiatePaymentConsentResponse == null) {
            return createUrl(result);
        }
        OBWriteDomesticConsentResponse1 consentResponse = readConsentResponse(initiatePaymentConsentResponse);
        return createUrl(result, consentResponse.getData().getConsentId(), initiatePaymentConsentResponse.getConsentUri());
    }

    private OBWriteDomesticConsentResponse1 readConsentResponse(final InitiatePaymentConsentResponse initiatePaymentConsentResponse) {
        try {
            return objectMapper.readValue(initiatePaymentConsentResponse.getPaymentConsent(), OBWriteDomesticConsentResponse1.class);
        } catch (JsonProcessingException e) {
            throw new IllegalStateException(e);
        }
    }

    private String createUrl(YoltBankUkInitiateSinglePaymentPreExecutionResult result, String consentId, String consentUri) {
    ...
    private String createUrl(YoltBankUkInitiateSinglePaymentPreExecutionResult result) {
