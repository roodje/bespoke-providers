...
public class YoltBankUkAuthorizationUrlPeriodicPaymentExtractorTest {

    @InjectMocks
    private YoltBankUkAuthorizationUrlPeriodicPaymentExtractor urlExtractor;
    ...
    void shouldReturnAuthorizationUrlCreatedBasedOnPreExecutionResultForExtractAuthorizationUrlWhenHttpResponseBodyIsNull() throws InvalidJwtException {
        ...
        // when
        String result = urlExtractor.extractAuthorizationUrl(null, yoltBankUkInitiatePeriodicPaymentPreExecutionResult);
    ...
    void shouldUseProperlyCreatedSignatureForExtractAuthorizationUrl() throws InvalidJwtException {
        ...
        // when
        urlExtractor.extractAuthorizationUrl(null, yoltBankUkInitiatePeriodicPaymentPreExecutionResult);
    ...
    void shouldReturnAuthorizationUrlApplyingConsentDataForExtractAuthorizationUrlWhenHttpResponseBodyIsNotNull() throws JsonProcessingException, InvalidJwtException {
        ...
        String vaseClientRedirectUrl = "http://localhost/redirect";
        YoltBankUkInitiatePeriodicPaymentPreExecutionResult yoltBankUkInitiatePeriodicPaymentPreExecutionResult = new YoltBankUkInitiatePeriodicPaymentPreExecutionResult(
                null,
                new PaymentAuthenticationMeans(clientId, signingKid, publicKid),
                signer,
                vaseClientRedirectUrl,
                state,
                null,
                null
        );
        ...
        OBWriteDomesticStandingOrderConsentResponse1 domesticConsentResponse1 = new OBWriteDomesticStandingOrderConsentResponse1()
                .data(new OBWriteDataDomesticStandingOrderConsentResponse1()
                        .consentId(paymentId));
        given(objectMapper.readValue(paymentConsent, OBWriteDomesticStandingOrderConsentResponse1.class))
                .willReturn(domesticConsentResponse1);
        given(signer.sign(jsonWebSignatureArgumentCaptor.capture(), eq(signingKid), eq(SignatureAlgorithm.SHA256_WITH_RSA)))
                .willReturn(jwsSigningResult);
        String compactSerialization = "compactSerialization";
        given(jwsSigningResult.getCompactSerialization())
                .willReturn(compactSerialization);

        // when
        String result = urlExtractor.extractAuthorizationUrl(initiatePaymentConsentResponse, yoltBankUkInitiatePeriodicPaymentPreExecutionResult);
        ...
        assertThat(uriComponents.getQueryParams().toSingleValueMap())
                .containsExactlyInAnyOrderEntriesOf(Map.of(
                        "client_id", clientId.toString(),
                        "token", compactSerialization,
                        "redirect_uri", vaseClientRedirectUrl,
                        "state", state
                ));
    }
}
