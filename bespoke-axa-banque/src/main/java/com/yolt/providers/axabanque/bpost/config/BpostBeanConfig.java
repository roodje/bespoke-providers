...
@Configuration
public class BpostBeanConfig {

    private DefaultHttpClientProducer getBpostHttpClientProducer(BpostProperties properties, MeterRegistry registry) {
        ...
        return new DefaultHttpClientProducer(
                ...
                properties,
                "v1",
                BpostDetailsProvider.PROVIDER_DISPLAY_NAME);
    }

    private AuthenticationService getBpostAuthenticationService(final Clock clock, BpostProperties properties, HttpClientProducer restTemplateProducer, ObjectMapper objectMapper) {
    ...
    private FetchDataService getBpostFetchDataService(BpostProperties properties, HttpClientProducer restTemplateProducer, ZoneId zoneId, Clock clock) {
    ...
    @Bean("BpostDataProviderV1")
    public UrlDataProvider getBpostDataProvider(@Qualifier("AxaGroupObjectMapper") ObjectMapper objectMapper,
                                                BpostProperties properties,
                                                Clock clock,
                                                MeterRegistry registry) {
        HttpClientProducer restTemplateProducer = getBpostHttpClientProducer(properties, registry);
        AuthenticationService authenticationService = getBpostAuthenticationService(clock, properties, restTemplateProducer, objectMapper);
        ZoneId zoneId = ZoneId.of("Europe/Brussels");
        FetchDataService fetchDataService = getBpostFetchDataService(properties, restTemplateProducer, zoneId, clock);
        return new GroupDataProvider(
                ...
                objectMapper,
                BpostDetailsProvider.PROVIDER_KEY,
                BpostDetailsProvider.PROVIDER_DISPLAY_NAME,
                VERSION_1);
    }
}
