...
class DefaultSubmitPaymentStatusesExtractorTest {

    private DefaultSubmitPaymentStatusesExtractor sut;
    ...
    void shouldReturnCorrectlyMappedPaymentStatusesForKnownStatusValue(final String status, final EnhancedPaymentStatus enhancedPaymentStatus) {
        // given
        var response = new PaymentStatusResponse(status);
        sut = new DefaultSubmitPaymentStatusesExtractor(new DefaultCommonPaymentStatusMapper());

        // when
        PaymentStatuses result = sut.extractPaymentStatuses(response, null);
