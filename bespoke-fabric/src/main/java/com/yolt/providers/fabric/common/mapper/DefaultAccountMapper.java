...
public class DefaultAccountMapper implements AccountMapper {
    ...
    private BigDecimal findBalance(final List<Balance> balances,
        ...
        if (!ObjectUtils.isEmpty(balances)) {
            ...
            return firstBalance == null ? null : BigDecimal.valueOf(firstBalance.getBalanceAmount().getAmount());
        } else {
            return null;
        }
    }
    ...
    private List<BalanceDTO> mapBalances(List<Balance> balances) {
        return balances.stream()
                .map(balance -> BalanceDTO.builder()
                        .balanceAmount(new BalanceAmountDTO(currencyCodeMapper.mapToCurrencyCode(balance.getCurrency()), BigDecimal.valueOf(balance.getBalanceAmount().getAmount())))
    ...
    private ZonedDateTime toNullableZonedDateTime(String dateTime) {
        if (StringUtils.isNotEmpty(dateTime)) {
            return ZonedDateTime.of(LocalDateTime.parse(dateTime, DateTimeFormatter.ISO_DATE_TIME), zoneId);
        }
        return null;
    }
