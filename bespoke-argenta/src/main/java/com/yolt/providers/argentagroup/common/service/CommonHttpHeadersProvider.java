...
public class CommonHttpHeadersProvider {
    ...
    public HttpHeaders provideCommonHeaders(final DefaultAuthenticationMeans authenticationMeans,
                                            final String psuIpAddress,
                                            final Signer signer,
                                            final Object requestBody) {
        HttpHeaders headers = new HttpHeaders();
        headers.set(X_REQUEST_ID_HEADER_NAME, externalTracingIdProvider.get());
        headers.set(DIGEST_HEADER_NAME, digestProducer.calculateSHA256Digest(requestBody));
        ...
        headers.set(PSU_IP_ADDRESS_HEADER_NAME, psuIpAddress);
