...
class RabobankSepaSubmitPaymentStatusesExtractorTest {

    private RabobankSepaSubmitPaymentStatusesExtractor subject;

    @BeforeEach
    void setUp() {
        subject = new RabobankSepaSubmitPaymentStatusesExtractor(new RabobankSepaPaymentStatusesMapper());
    }
    ...
    void shouldReturnPaymentStatuses(TransactionStatus bankStatus, EnhancedPaymentStatus newStatus) {
        ...
        statusResponse.setTransactionStatus(bankStatus);

        //when
        PaymentStatuses mappedStatuses = subject.extractPaymentStatuses(statusResponse, null);
