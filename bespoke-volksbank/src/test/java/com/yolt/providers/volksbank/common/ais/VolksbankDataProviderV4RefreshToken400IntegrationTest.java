...
@AutoConfigureWireMock(stubs = "classpath:/stubs/volksbank/api_1.1/ais/refresh_token_400", port = 0, httpsPort = 0)
@ActiveProfiles("volksbank")
public class VolksbankDataProviderV4RefreshToken400IntegrationTest {
    ...
    private static final String EXPIRED_REFRESH_TOKEN = "4a33007a-0000-1111-0000-0242ac130003";
    ...
    public void shouldThrowTokenInvalidExceptionWhenReceived400WithUnsupportedGrantTypeMessage(UrlDataProvider dataProviderUnderTest) throws IOException {
        //given
        AccessMeansDTO accessMeansDTO = createAccessMeansDTO(createVolksbankAccessMeans());
        ...
        // then
        assertThatExceptionOfType(TokenInvalidException.class).isThrownBy(throwingCallable);
    }

    @ParameterizedTest
    @MethodSource("getVolksbankProviders")
    public void shouldThrowTokenInvalidExceptionWhenReceived400WithExpiredRefreshTokenMessage(UrlDataProvider dataProviderUnderTest) throws IOException {
        //given
        VolksbankAccessMeans volksbankAccessMeans = createVolksbankAccessMeans();
        volksbankAccessMeans.getResponse().setRefreshToken(EXPIRED_REFRESH_TOKEN);
        AccessMeansDTO accessMeansDTO = createAccessMeansDTO(volksbankAccessMeans);
    ...
    private AccessMeansDTO createAccessMeansDTO(VolksbankAccessMeans volksbankAccessMeans) throws JsonProcessingException {
        return new AccessMeansDTO(
                UUID.randomUUID(),
                mapper.writeValueAsString(volksbankAccessMeans),
                new Date(),
                Date.from(Instant.now().plusSeconds(600)));
    }

    private VolksbankAccessMeans createVolksbankAccessMeans() {
